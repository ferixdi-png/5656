...
## ✅ Ручные сценарии проверки (бот)

1) **Image модель:** `/start` → “🚀 Быстрый старт (3 шага)” → TikTok → Продажи → Картинка → выбрать инструмент → ввести prompt → ✅ Подтвердить → получить ссылку на изображение.
2) **Video модель:** `/start` → “🎬 Видео для соцсетей” → выбрать платформу/цель/формат → выбрать инструмент → заполнить inputs → ✅ Подтвердить → получить ссылку на видео.
3) **File модель:** `/start` → “🎨 Креативы/баннеры” → выбрать инструмент с file input → отправить фото → ✅ Подтвердить → получить результат или понятную ошибку.
