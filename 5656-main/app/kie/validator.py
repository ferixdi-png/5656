...
def validate_input_type(value: Any, expected_type: str, field_name: str, is_required: bool = True) -> None:
    ...
    if expected_type in ['file', 'file_id', 'file_url']:
        ...
        # ИСКЛЮЧЕНИЕ: Для image-edit моделей prompt может быть пустой строкой даже если required
        if is_required and not value.strip():
            # CRITICAL FIX: Для image-edit моделей разрешаем пустой prompt
            # Проверяем по имени поля, а не по model_id (т.к. model_id недоступен здесь)
            if field_name == 'prompt':
                # Для prompt в image-edit моделях пустая строка допустима
                # Это обрабатывается на уровне validate_model_inputs
                pass  # Разрешаем пустой prompt
            else:
                raise ModelContractError(
...
def validate_model_inputs(
    ...
    if 'input' in input_schema and isinstance(input_schema['input'], dict):
        ...
        # ВАРИАНТ 2: input имеет examples (описание поля) - большинство моделей
            ...
            if examples and isinstance(examples[0], dict):
                ...
                # ИСКЛЮЧЕНИЕ: Для image-edit моделей prompt всегда должен присутствовать (даже если пустой)
                if 'prompt' in input_schema:
                    prompt_value = example_structure.get('prompt', '')
                    # CRITICAL FIX: Для image-edit моделей prompt всегда required (даже если может быть пустым)
                    if 'image-edit' in model_id.lower() or 'image_edit' in model_id.lower():
                        input_schema['prompt']['required'] = True
                        logger.debug(f"Image-edit model {model_id}: prompt is required (can be empty string)")
    ...
    for field_name in required_fields:
        if field_name not in user_inputs:
            ...
            # CRITICAL FIX: Для image-edit моделей prompt может быть пустой строкой, но должен присутствовать
            if value is None and field_name == 'prompt' and ('image-edit' in model_id.lower() or 'image_edit' in model_id.lower()):
                # Для image-edit моделей добавляем пустой prompt если отсутствует
                logger.debug(f"Image-edit model {model_id}: adding empty prompt as default")
                user_inputs['prompt'] = ""
                value = ""
    ...
    for field_name in all_fields:
        ...
        if value is not None:
            ...
            # Only enforce enum for non-text fields (like mode, quality, style, etc.)
            if 'enum' in field_spec:
                enum_values = field_spec['enum']
                # Text fields: enum is optional (examples/suggestions)
