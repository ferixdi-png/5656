...
async def cb_history_main(callback: CallbackQuery, state: FSMContext, cid=None, bot_state=None):
    ...
    if not callback.from_user:
        logger.error("[HISTORY] callback.from_user is None in cb_history_main")
    ...
    if not callback.message:
        logger.error("[HISTORY] callback.message is None in cb_history_main")
...
async def cb_history_transactions(callback: CallbackQuery, state: FSMContext, cid=None, bot_state=None):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="◀️ Назад к генерациям", callback_data="main_menu")]
    ])
